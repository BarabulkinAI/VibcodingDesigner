...
- **Убрать кнопку «Обновить превью»** — превью и так обновляется автоматически на
  каждую зафиксированную мутацию документа (`DesignSurfaceViewModel.DocumentChanged`
  → `MainViewModel.RefreshPreview()`, см. `OnDesignSurfaceDocumentChanged`), кнопка
  в тулбаре (`MainWindow.axaml`, `OnRefresh`) — не нужный сейчас ручной дубль,
  оставшийся с более ранних этапов.
