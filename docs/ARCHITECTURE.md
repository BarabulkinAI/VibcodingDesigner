...
- **Курсор при изменении размера объекта** — сейчас `DesignSurface.cs` меняет курсор
  только на `Cross` при активном инструменте тулбокса (создание объекта, строка 77);
  при наведении на ручку ресайза выделенного объекта или во время самого ресайза
  курсор не меняется (не показывает направление — ↔/↕/↖↘ и т.п.).
- **Кнопка в верхнем меню — скрыть панель превью** — сейчас панель превью всегда
  видима, отдельного переключателя видимости нет.
