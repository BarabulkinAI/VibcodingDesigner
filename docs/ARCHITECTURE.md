...
4. Источники данных, заданные в дизайнере (этап 3), — это примерные данные для превью,
   ...
   **Уточнение (найдено на этапе 4 при ручном тестировании — реальный краш приложения,
   не гипотетический edge case):** хотя источник данных не сохраняется как *данные*, сама
   привязка `DataBand.DataSource` (и компонент `TableDataSource` без данных) сериализуется
   в `.frx` как часть графа отчёта. После `Load()` это давало `DataTableException: Table is
   not connected to the data` прямо в `DataBand.InitDataSource()` — падало вообще без
   единого `[Field]`-выражения, просто от факта, что полоса когда-то была привязана к
   источнику. Исправлено: `Load()`/`LoadAsTemplate()` теперь явно отвязывают и удаляют
   (`Dispose()`) все унаследованные из файла источники (см. факт про `ClearRegisteredData()`
   выше). Но если в тексте объекта остался буквальный `[Источник.Колонка]` (а не только
   привязка полосы) — `Report.Prepare()` всё равно падает, теперь уже с
   `CompilerException: CS0103` (FastReport компилирует выражения в C#, необъявленный
   идентификатор — ошибка компиляции, а не мягкая деградация в текст плейсхолдера). Это
   *ожидаемое* следствие того же самого ограничения (данные не персистентны), а не отдельный
   баг — но раз оно неизбежно возникает при обычном цикле «добавили источник → сохранили →
   закрыли → открыли», `Report.Prepare()` в приложении **не должен вызываться без try/catch**:
   `MainViewModel.RefreshPreview()`/экспорт теперь ловят исключение и показывают
   `PreviewErrorMessage` вместо падения всего приложения (превью просто недоступно, а не
   краш). Полноценное решение (перестать терять `[Field]`-выражения на несуществующие
   источники) потребовало бы либо персистентности источников, либо очистки битых выражений
   при `Load()` — оба варианта вне рамок этапа 4, зафиксировано как известное ограничение.